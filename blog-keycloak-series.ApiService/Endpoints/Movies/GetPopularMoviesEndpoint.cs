...
public static class GetPopularMoviesEndpoint
{
    private const string Name = "GetPopularMovies";
    
    public static IEndpointRouteBuilder MapGetPopularMoviesEndpoint(this IEndpointRouteBuilder app, ApiVersionSet versionSet)
    {
        app.MapGet(ApiEndpoints.Movies.Popular, async Task<Results<Ok<ApiResponse>, BadRequest<List<string>>>>
        ([FromServices] HybridCache cache, [FromServices] IHttpClientFactory httpClientFactory, CancellationToken token) =>
        {
            ...
            var response = await httpClient.GetAsync("movie/popular?language=en-US&page=1", token);
