...
public static class GetTopRatedMoviesEndpoint
{
    private const string Name = "GetTopRatedMovies";
    
    public static IEndpointRouteBuilder MapGetTopRatedMoviesEndpoint(this IEndpointRouteBuilder app, ApiVersionSet versionSet)
    {
        app.MapGet(ApiEndpoints.Movies.TopRated, async Task<Results<Ok<ApiResponse>, BadRequest<List<string>>>>
        ([FromServices] HybridCache cache, [FromServices] IHttpClientFactory httpClientFactory, CancellationToken token) =>
        {
            ...
            var response = await httpClient.GetAsync("movie/top_rated?language=en-US&page=1", token);
